...
function createRouter(init) {
  ...
  function fetch2(key, routeId, href, opts) {
    ...
    if (submission && isMutationMethod(submission.formMethod)) {
      handleFetcherAction(key, routeId, path2, match, matches, fogOfWar.active, flushSync, preventScrollReset, submission);
      return;
    }
    ...
    handleFetcherLoader(key, routeId, path2, match, matches, fogOfWar.active, flushSync, preventScrollReset, submission);
  }
  async function handleFetcherAction(key, routeId, path2, match, requestMatches, isFogOfWar, flushSync, preventScrollReset, submission) {
    interruptActiveLoads();
  ...
  async function handleFetcherLoader(key, routeId, path2, match, matches, isFogOfWar, flushSync, preventScrollReset, submission) {
    let existingFetcher = state.fetchers.get(key);
...
_Constants.VIEW_COMPONENT_TYPE_PROFILE_PIC = "ProfilePic";
_Constants.VIEW_COMPONENT_TYPE_LINK_LIST = "LinkList";
_Constants.VIEW_COMPONENT_TYPE_TEXT_BOX = "TextBox";
_Constants.POST_STATUS_PUBLISH = "publish";
_Constants.POST_STATUS_DRAFT = "draft";
let Constants = _Constants;
function Loading({ loaded }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex justify-center mt-20 " + (loaded ? "hidden" : "visible"), children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "loading loading-bars loading-lg text-primary" }) });
}
function ok$1() {
}
