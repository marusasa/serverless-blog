	...
	public static void getPageEngagement(Context ctx) {
		...
		try {
			...
			if(result.getMessages().size() == 0) {
				final List<PathSummary> list = AnalyticsLogic.getPageEngagementSummaryByPath(year, month);
	...
	public static void getDailyVisits(Context ctx) {
		...
		try {
			...
			if(result.getMessages().size() == 0) {		
				...
				final List<CloudDocument> list = AnalyticsLogic.getDailyVisits(year, month);
				list.forEach(doc -> {
