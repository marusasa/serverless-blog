...
public class ArticleGetListByTag implements Handler {
	...
	public void handle(@NotNull Context ctx) throws Exception {
		...
		try {
			...
			Optional<CloudDocument> o = Env.tagDao.getTag(tagId);
			if(o.isPresent()) {
			...
			if(result.getMessages().size() == 0) {
				final List<CloudDocument> documents = Env.articleDao.getArticlesByTag(tagId);
				for (CloudDocument doc : documents) {
					Optional<CloudDocument> op = Env.articleDao.getArticleForManage(doc.getString(ArticleTagDoc.field_article_id));
					if(op.isPresent()) {
						...
						if(docArticle.getString(ArticleDoc.field_status).equals(AppConst.ART_STATUS_PUBLISH)) {
							final var article = new Article.Builder()
									...
									.publishedAt(Env.getJavaScriptUtcDateTime(docArticle, ArticleDoc.field_published_at))
